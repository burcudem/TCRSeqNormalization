...
### uses meso samples from DNA161003LC S39-S50
...
### I/O
inputDir_v <- file.path(baseDir_v, "./data/mesoTumor/25bp/")
outDir_v <- file.path(baseDir_v, "./output/suppFig4/")
sf_dt <- fread(file.path(baseDir_v, "./meta/nb.scaling.factors.txt"))
...
inputData_dt <- inputData_dt[,mget(c("VJ", samples_v))]
...
inputData_dt[, (samples_v) := lapply(.SD, as.numeric), .SDcols = samples_v]
...
for (i in 1:nrow(sf_dt)) {
  ...
  for (j in 1:length(samples_v)) {
    
    ## Get sample
    currSample_v <- samples_v[j]
...
### Print
pdf(file = file.path(outDir_v, "suppFig4.pdf"), width = 9, height = 9)
print(plot_gg)
graphics.off()
